...
/* -----------------------------------------------------------------------
 * witaiinterface_gateway.php
 * -----------------------------------------------------------------------
 * Purpose : TV routines for interface with Wit.ai via gateway
 * Author  : Pitichat Suttaroj <pitichat@tellvoice.com>
 * Created : 10 Nov 2017
 * History :
 *  10 Nov 2017 - Create file
...
define('LOGFILE', '../log/log_utils/witaiinterface_gateway_'.date('Ymd').'.log');
...
if(empty($accessToken)){
	$outputJSON = array();
	$outputJSON['status'] = "FAIL";    $outputJSON['description'] = "Invalid request content(No 'accesstoken')";
	$outputJSON['httpcode'] = "";      $outputJSON['responsetext'] = "";
	/* Output */
	http_response_code(400);
	header('Content-Type: application/json');
	echo json_encode($outputJSON);
	logWrite(LOGFILE, "Invalid request content(No 'accesstoken')");
	exit(-1);
}
...
$outputJSON = witAIInterface($httpMethod, $url, $queryStrArray, $accessToken, $contentText, $timeout);
...
if($outputJSON['status'] != "SUCCESS"){
	$httpResponseCode = empty($outputJSON['httpcode'])?404:$outputJSON['httpcode'];
	http_response_code($httpResponseCode);
}

header('Content-Type: application/json');

$responseString = $outputJSON['responsetext'];

echo $responseString;

logWrite(LOGFILE, "HTTP code = ".$httpResponseCode." , Response string = \"".$responseString."\"");
...
/* witAIInterface
 * --------------
 * Purpose : Wit.ai HTTP interface function
 * Input   :
 *  (1) '$httpMethod' = input HTTP method("GET"/"POST"/"PUT"/"DELETE")
 *  (2) '$url' = Wit.ai URL(base)
 *  (3) '$queryStrArray' = query string array
 *  (4) '$accessToken' = Wit.ai access token
 *  (5) '$contentText' = outgoing content text(JSON) to Wit.ai
 *  (6) '$timeout' = Wit.ai response timeout in second
 * Return  : Array of...
 *  (1) $retVal['status'] = Status of this function(SUCCESS/FAIL?)
 *  (2) $retVal['description'] = Status description
 *  (3) $retVal['httpcode'] = Returned HTTP status from Wit.ai
 *  (4) $retVal['responsetext'] = Response from Wit.ai
 */

function witAIInterface($httpMethod, $url, $queryStrArray, $accessToken, $contentText, $timeout)
{
	$retVal = array();
	...
	logWrite(LOGFILE, "witAIInterface: URL = \"".$url."\"");
	logWrite(LOGFILE, "witAIInterface: contentText = \"".$contentText."\"");
