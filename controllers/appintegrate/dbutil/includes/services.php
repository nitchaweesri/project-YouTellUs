...
if(file_exists('dbutil/tvssdefineparams.php')){
    include_once('dbutil/tvssdefineparams.php');
}else if(file_exists('../dbutil/tvssdefineparams.php')){
    include_once('../dbutil/tvssdefineparams.php');
}else if(file_exists('../../dbutil/tvssdefineparams.php')){
    include_once('../../dbutil/tvssdefineparams.php');
}else if(file_exists('../../../dbutil/tvssdefineparams.php')){
...
$dec_pswd = openssl_decrypt(DEFAULT_PSWD, 'AES-256-CBC', AUTH_KEYS, 0, '19-L4aw7r8]l5T2U');
define('DBCONFIG_PSWD', $dec_pswd);
...
function getResultSet($SQLStatement)
{
	...
	if(!$MainDB->Connect(DBCONFIG_SERVER, DBCONFIG_USER, DBCONFIG_PSWD, DBCONFIG_DBNAME)){
		print "Error MySQL Connection";
	}
	
	$MainDB->Execute('SET NAMES utf8mb4');
...
function getQueryResult($SQLStatement, $value = null){
	...
	// CREATE CONNECTION //
	$mysqli = new mysqli(DBCONFIG_SERVER, DBCONFIG_USER, DBCONFIG_PSWD, DBCONFIG_DBNAME);
...
function executeSQL_LastInsertId_Old($sqlStatement)
{
	$execInfo = array("isSuccess" => true, "errorDescp" => "", "lastId" => "");
		
	/* Create connection */
	$conn = mysqli_connect(DBCONFIG_SERVER, DBCONFIG_USER, DBCONFIG_PSWD, DBCONFIG_DBNAME);
...
/* Added by Pitichat 2018-07-02 */
function executeSQL_LastInsertId($sqlStatement, $value = null)
{
	$execInfo = array("isSuccess" => true, "errorDescp" => "", "lastId" => "");
	...
	/* Set UTF-8 */
	$statement = $mysqli->prepare('SET NAMES utf8mb4');
	$statement->execute();
	$statement->close();
	...
	/* Get insert id */
	$execInfo["lastId"] = $stmt->insert_id;
